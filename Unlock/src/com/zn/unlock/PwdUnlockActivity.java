...
public class PwdUnlockActivity extends Activity
	...
	public void onClick(View arg0) {
		...
		if(count==4){
			mImgView_passwd_show.setImageResource(R.drawable.passwd4);
				File file =new File("/sdcard/Unlock/passwd.txt");
				try {
					if(Util.readTxtFile(file).equals(passwdBuffer.toString())){
						PwdUnlockActivity.this.finish();
					}
					else{
						//System.out.println(Util.readTxt("/sdcard/Unlock/passwd.txt"));
						mImgView_passwd_show.setImageResource(R.drawable.passwd0);
						count=0;
						Toast.makeText(PwdUnlockActivity.this, "密码错误，请重新输入",Toast.LENGTH_LONG).show();
					}
				} catch (Exception e) {
